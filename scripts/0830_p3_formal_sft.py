...
OUT = Path(os.environ.get("P3_OUT", "/data/magnus/closedloop-0828/p3-formal-v1"))
...
def main() -> int:
    ...
    def evaluate_and_decide(trainer, step: int) -> None:
        ...
        if dual["format_rate"] < FORMAT_GATE:
            if state["last_dev_fmt"] is not None and state["last_dev_fmt"] < FORMAT_GATE:
                klog("K1", f"dev format {dual['format_rate']} < {FORMAT_GATE} twice (prev {state['last_dev_fmt']})")
                raise KillSwitch("K1")
        ...
        if recent_loss is not None and recent_loss < K3_LOSS:
            ...
            if state["flat_streak"] >= K3_FLAT_RUN and dual["format_rate"] < 1.0:
                klog("K3", f"loss<{K3_LOSS} and dev flat {K3_FLAT_RUN} checkpoints (memorization)")
                raise KillSwitch("K3")
        # I-08 flag
    ...
    t1 = time.time()
    marker("train", "start", epochs=EPOCHS, micro=MICRO, accum=ACCUM)
    status = "completed"
    try:
        trainer.train()
    ...
    (OUT / "merged-eval-arm").mkdir(parents=True, exist_ok=True)
    merged.save_pretrained(str(OUT / "merged-eval-arm"), safe_serialization=True)
    tok.save_pretrained(str(OUT / "merged-eval-arm"))
    RECEIPT["artifacts"] = {
        "eval_arm_rule": "dual-column lexicographic best checkpoint",
        "eval_arm_step": state["best"]["step"] if state["best"] else "final",
