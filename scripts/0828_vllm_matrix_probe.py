...
def main() -> int:
    ...
    record = {"schema": "r2-0-vllm-matrix/v4", "candidate_spec": args.candidate,
              ...
              "dependency_policy": "vllm/cloudpickle/pydantic/pydantic-core/typing-inspection/annotated-types --no-deps; preserve base-image torch",
    ...
    stdout, status, rc = run([str(py), "-m", "pip", "install", "--no-cache-dir", "--no-deps",
                              args.candidate, "cloudpickle", "pydantic==2.13.5", "pydantic-core==2.46.5", "typing-inspection==0.4.2", "annotated-types==0.7.0"], timeout=args.install_timeout)
